...
//
// Bindea a 127.0.0.1: wacli corre en la misma box que el canal, así que loopback alcanza y
// mantiene el webhook fuera de cualquier interfaz externa (wacli pide `--webhook-allow-private`
// para aceptar una URL loopback — lo agrega el spawn del follow en el canal).
...
export class WacliWebhookServer {
    ...
    return new Promise<void>((resolve, reject) => {
      const server = createServer((req, res) => {
        void this.handleRequest(req, res).catch((err) => {
          console.log(dim(`[whatsapp:webhook] handler crashed: ${String(err)}`));
          if (!res.headersSent) {
  ...
  private async handleRequest(
    ...
    if (!verifySignature(this.secret, body, signatureHeader)) {
      console.log(dim("[whatsapp:webhook] firma inválida — rechazado"));
    ...
    try {
      ...
      console.log(dim(`[whatsapp:webhook] payload no-JSON: ${String(err)}`));
    ...
    if (this.handler) {
      try {
        ...
        console.log(dim(`[whatsapp:webhook] handler threw: ${String(err)} (id ${msg.ID})`));
      }
    }
  }
}
